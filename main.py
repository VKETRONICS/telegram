...
async def startup_event():
    ...
    await send_daily_greeting()  # Временный вызов, чтобы увидеть кнопку сразу

async def send_daily_greeting():
    import random
    if GROUP_CHAT_ID and BOT_USERNAME:
        greetings = [
        ...
        text = random.choice(greetings) + "\n\n💬 Чтобы воспользоваться каталогом, получить помощь с выбором техники или задать вопрос, нажмите кнопку ниже и начните диалог со мной в личных сообщениях."
        inline_menu = {
            "inline_keyboard": [
                [{"text": "🚀 ОТКРЫТЬ ETRONICS ПОМОЩНИКА", "url": f"https://t.me/{BOT_USERNAME}?start=menu"}]
            ]
        }
        await send_message(int(GROUP_CHAT_ID), text, inline_menu)
